...
class ModelVerificationProcess(Process):
    def __init__(self, id: int, globalJobQueue: 'Queue', globalMsgQueue: 'Queue') -> None:
    ...
    def run(self) -> None:
        while True:
            try:
                ...
                self.globalMsgQueue.put((EnumMessageType.VERIFY_RESULT, res, runtime))
                #self.globalMsgQueue.put((EnumMessageType.VERIFY_RESULT, False, 1))
                #res = True
                if res == False:
                    break
            except Exception:
