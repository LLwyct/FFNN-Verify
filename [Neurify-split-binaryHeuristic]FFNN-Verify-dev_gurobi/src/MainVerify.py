...
class MainVerify:
    ...
    def verify(self):
        ...
        while True:
            try:
                res = self.messageQueue.get(timeout=GlobalSetting.TIME_OUT)
                # print("接到第一个线程的时间", timer() - start)
        ...
        if self.spec.verifyType == "mnist":
            return (finalSatResult, splitProcessTotalRuntime, solverProcessRuntime, max_jobs_num, end - start, isTimeout)
